...
from config import WORK_START, WORK_END, SLOT_STEP_MINUTES

TIMEZONE = ZoneInfo("Europe/Vienna")


def generate_time_slots(date):
    start = datetime(date.year, date.month, date.day, WORK_START, 0, tzinfo=TIMEZONE)
    end = datetime(date.year, date.month, date.day, WORK_END, 0, tzinfo=TIMEZONE)
